...
public class OspreyView extends GameView {
	...
	private final static int NUM_IMAGES = 35;
	private final static int NUM_ANIMATIONS = 20;
	...
	private void makeFrames() {
		animationFrames = new BufferedImage[NUM_ANIMATIONS];
		int numSets1 = 3;
		int offset1 = 1;
		for(int i = 0; i < numSets1; i++) {
			for(int j = 0; j < NUM_FRAMES; j++){
				animationFrames[j + NUM_FRAMES*i] = images[offset1 + i].getSubimage(IMAGE_WIDTH*j, 0, IMAGE_WIDTH, IMAGE_HEIGHT);
			}
		}
		int numSets2 = 2;
		int offset2 = 33;
		for(int i = 0; i < numSets2; i++) {
			for(int j = 0; j < NUM_FRAMES; j++){
				animationFrames[j + NUM_FRAMES*i + 12] = images[offset2 + i].getSubimage(IMAGE_WIDTH*j, 0, IMAGE_WIDTH, IMAGE_HEIGHT);
			}
		}
	}
